...
namespace ConferencySystem.ViewModels.Admin
{
    ...
    public class PeopleViewModel : MainMasterPageViewModel
    {
        ...
        public BusinessPackDataSet<AppUserDTO> People { get; set; } = new BusinessPackDataSet<AppUserDTO>();

        public override Task PreRender()
        {
            People.PagingOptions.PageSize = 500;
            People.SortingOptions.SortExpression = nameof(AppUserDTO.Id);
            People.SortingOptions.SortDescending = false;

            var adminService = new AdminService();
            adminService.GetPersons(People);

            TotalPeopleCount = People.PagingOptions.TotalItemsCount;
        ...
        public void SendEmails()
        {
            ...
            List<AppUserDTO> allPeople = loginService.GetAllPeople();
            ...
            foreach (AppUserDTO person in allPeople)
            {
                MailBodyhtml =
                ...
                var msg = new MailMessage(Secrets.GmailZN, person.Email, "Nakopnìte svoji školu - registrace workshopù", MailBodyhtml);
                msg.Bcc.Add(new MailAddress("martinbedn20@seznam.cz"));
                msg.ReplyToList.Add(new MailAddress(Secrets.GmailZN));
                msg.IsBodyHtml = true;
                smtpClient.Send(msg);

                loginService.EmailSent(person.Id);
            }
        }
